...
本次更新标志着 CosyVoice 架构的重大整合。

#### ✅ 已完成 (Perfectly Supported)
- **V3 系列 (0.5B)**: 完美支持。性别 (Gender)、方言 (Dialect)、情感 (Emotion) 以及指令文本 (Instruct) 均能精确跟随。音质极佳。
- **V2 系列 (0.5B)**: 完美支持。与 V3 表现一致，稳定性极高。
- **V1 300M / 300M-SFT**: 恢复原生表现。固定音色（Identity）推理稳定。
- **稳定性**: 彻底解决 `AudioDecoder` 和 `KeyError` 等早期崩溃问题。

#### 🚧 待优化 (In Progress)
- **300M-Instruct**: 目前虽集成了专用指令路径，但在特定环境下（如缺少 RL 权重时）其指令遵循的一致性仍有待提升。生成结果有时会偏移为普通话女声。

#### 🔈 资源更新
- **HQ 种子音色**: 现已内置高保真男声/女声种子音频。当模型进入 Recovery 模式（V3 缺内置音色且无参考音频）时，将自动使用 HQ 种子作为基准，显著提升生成质感。
