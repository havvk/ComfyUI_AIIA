...
- **高级参数 (Advanced Parameters)**:
  ...
  - `blink_mode`: (默认 Random) **眨眼模式控制**。
    - `Random (Normal)`: 自然随机眨眼 (60-100帧/次)。
  ...
  - `relax_on_silence`: (默认 True) **静音归位 (Relax Face on Silence)**。
    ...
    - **智能抗抖动**: 内置信号稳定算法，自动填补语音中的微小停顿，消除因音量波动导致的“嘴巴频繁开合”问题。
    - 该功能**仅针对嘴部表情**，不会冻结头部运动和眨眼，确保人物在不说话时依然保持自然生气。
  - `ref_threshold`: (默认 0.005) 静音检测阈值。低于此音量的音频将被视为静音并触发归位机制。
