...
- **高级参数 (Advanced Parameters)**:
  ...
  - `blink_mode`: (默认 Random) **眨眼模式控制**。
    - `Random (Normal)`: **拟人化随机眨眼**。
      - 基础频率：60-100帧/次。
      - **双眨眼 (Double Blink)**: 约 25% 概率触发快速连续眨眼（间隔2-6帧），模拟真实人类的生理特征，大幅提升灵动感。
  ...
  - `relax_on_silence`: (默认 True) **静音归位 (Relax Face on Silence)**。
    - 针对静音片段进行**平滑过渡**（慢速闭合），避免“紧绷抿嘴”；
    - 对新语音信号**快速响应**（0.2秒内线性开启），确保发音口型自然且无延迟，同时也避免了瞬间跳变导致的“急促感”。
    - **智能抗抖动 (Stabilization)**: 内置信号稳定算法，自动填补语音中的微小停顿（<0.3s），消除因音量波动导致的“嘴巴频繁开合”问提。
    - **防止累积误差 (Drift Correction)**:
      - **精密网格对齐 (Exact Onset Alignment)**: 将模型的时间步长精确对齐到语音的开始（Onset），而不是固定的处理块。
      - **状态重置 (State & RNG Reset)**: 在长静音后，强制重置模型状态和**随机种子**，确保每一段新语音都拥有“第一段”般的生成质量。
  ...
  - `mouth_amp`: **嘴部幅度缩放** (0.0 - 2.0)。
    - 默认为 `1.0` (标准幅度)。
    - **解决张嘴图问题**: 如果原图是张着嘴的，生成的视频可能会导致嘴巴张得过大。此时将此值调小 (例如 `0.5` - `0.8`) 可以有效抑制嘴部动作，使其更自然。
- **输出**:
