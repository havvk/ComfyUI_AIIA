...
- **节点参数 (推荐参数建议)**:
  - `cfg_scale` (默认: 3.0): CFG 引导强度。
    - **官方基准 (Official Baseline)**: **1.3** (这是官方默认值)。
    - **1.5B 模型**: 建议 **3.0-5.0** 以获得极致音效。
    - **7B 模型**: 建议使用官方值 **1.3 - 2.0**。超过 3.5 可能导致音质下降。
  - `ddpm_steps` (默认: 50): 扩散步数。建议 20-50。
  - `do_sample` (默认: True): **7B 模型强烈建议开启**。如果不开启（False），模型将使用贪婪搜索，音色会比较呆板。
  - `temperature` (默认: 0.8): 采样温度。推荐 0.7 - 1.0。
  - `top_k / top_p`: 采样约束，保持默认即可。
  - `speed` (默认: 1.0): 后处理播放速度调整。
  - `normalize_text` (默认: True): 文本预处理补丁。
    - **1.5B**: 强烈建议开启。
