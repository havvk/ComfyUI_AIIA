...
- **节点参数 (推荐参数建议)**:
  - `cfg_scale` (默认: 3.0): CFG 引导强度。
    - **官方基准 (Official Baseline)**: **1.3** (这是官方代码中的默认值，也是最稳定的)。
    - **1.5B 模型**: 建议 **3.0-5.0** 以获得极致音质（1.5B 在高 CFG 下表现出奇地好）。
    - **7B 模型**: 建议使用官方值 **1.3 - 2.0**。超过 3.5 极易导致 7B 模型产生破音或电音。
  - `ddpm_steps` (默认: 50): 扩散步数。
