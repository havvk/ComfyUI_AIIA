...
class WanFunInpaintAudioPipeline(DiffusionPipeline):
    ...
    def prepare_latents(
        ...
        if latents is None:
            # Force latents to be on the same device as reference control (transformer)
            # regardless of what the pipeline thinks it is.
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    if do_classifier_free_guidance:
                        ...
                        # Fix: Interleave [N, P, P] for each batch item separately (Flat List)
                        in_prompt_embeds = []
                        for n, p in zip(negative_prompt_embeds, prompt_embeds):
                            in_prompt_embeds.extend([n, p, p])
                    else:
                        in_prompt_embeds = prompt_embeds
